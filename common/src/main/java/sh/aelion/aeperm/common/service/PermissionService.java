...
public final class PermissionService implements AepermAPI {
    ...
    public Optional<String> getPrimaryGroup(UUID uuid) {
        UserData user = loadOrCreateUser(uuid);
        return Optional.ofNullable(user.primaryGroup());
    }
    ...
    @Override
    public Set<String> groupNamesFilter(String like) {
        String lower = like.toLowerCase(Locale.ROOT);
        return new HashSet<>(groupNames().stream()
    ...
    public Set<String> userGroups(UUID uuid) {
        UserData user = loadOrCreateUser(uuid);
        Set<String> groups = new HashSet<>(user.groups());
        for (UserData.TempMembership temp : user.tempMemberships()) {
            if (temp.expiry().isAfter(clock.instant())) {
    ...
    private GroupData loadOrCreateGroup(String group) {
        ...
        return cached.orElseGet(() -> storage.loadGroup(name).orElseGet(() -> {
